...
const EventCard = ({event}) => {
	const {id, title, thumbnail, price, description} = event;
	...
	return (
		<>
			<div className="card lg:card-side bg-base-100 shadow-xl border rounded-none">
				<figure className="">
					<img
						className="lg:w-72 h-full w-full object-cover"
						src={thumbnail}
						alt=""
					/>
				</figure>
				<div className="card-body flex-1">
					<h2 className="card-title text-2xl font-lora mb-2">{title}</h2>
					...
					<div className="card-actions mt-2">
						<Link
							to={`/event/${id}`}
							className="btn btn-neutral btn-outline text-lg capitalize">
							Learn More
						</Link>
					</div>
				</div>
